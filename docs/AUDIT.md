...
**68 checks, 0 failures.** Every one of them is executed by
...
| 3. offline suite | 4 | 310 tests pass with **0 failures and 0 skips**; the suite is ≥300; the stub raises on any network or model access; fixtures are regenerable |
...
| 7. method surface | 25 | every method the README documents exists in the contract |
| 8. live deployment | 9 | deployed source byte-identical to the artifact; fee 0; `verify_risk` returns true; `require_safe` reverts; `is_safe` is false when unscanned; consumer reads the oracle cross-contract; `preview_dependency` degrades; `gate_build` fails a mixed list |
